(function () {
  ...
  try {
    ...
    const PAGE_IDENTIFIER_SELECTOR =
    ...
    function addBookmarkletStyles() {
      try {
        ...
        logToMemory("Estilos V13 injetados.", "debug");
      } catch (e) {
        console.error(`${SCRIPT_NAME} StyleErr:`, e);
        logToMemory(`Erro ao injetar estilos: ${e}`, "error");
      }
    }
    function createToastContainer() {
      if (!document.getElementById("hck-toast-container")) {
        toastContainer = document.createElement("div");
        toastContainer.id = "hck-toast-container";
        document.body.appendChild(toastContainer);
      } else {
        toastContainer = document.getElementById("hck-toast-container");
      }
    }
    function showToast(message, type = "info", duration = TOAST_DURATION) {
      if (!toastContainer) createToastContainer();
      const t = document.createElement("div");
      t.className = "hck-toast";
    ...
    function logToMemory(message, type = "info") {
      const ts = new Date();
      const e = { timestamp: ts, type, message };
      logArray.push(e);
      if (type !== "debug")
        console[type === "error" ? "error" : "log"](
          `[${formatTime(ts)}] ${type.toUpperCase()}: ${message}`
        );
      if (logPanelVisible && logContentDiv) renderSingleLogEntry(e);
    }
    function updateStatus(message, type = "info", showToastFlag = false) {
      if (statusLine) {
        ...
        else if (type === "success") statusLine.classList.add("success");
      }
      const lt =
    ...
    function renderSingleLogEntry(e) {
      if (!logContentDiv) return;
      const d = document.createElement("div");
      d.className = `hck-log-entry ${e.type}`;
      ...
      d.appendChild(t);
      d.appendChild(c);
      logContentDiv.appendChild(d);
      logContentDiv.scrollTop = logContentDiv.scrollHeight;
    }
    function renderLogs() {
      if (!logContentDiv) return;
      logContentDiv.innerHTML = "";
      logArray.forEach(renderSingleLogEntry);
    }
    function clearLogs() {
      logArray = [];
      renderLogs();
      logToMemory("Logs do painel limpos.", "info");
      showToast("Logs do painel limpos");
    }
    function createLogPanel() {
      if (document.getElementById("hck-log-panel")) return;
      logPanel = document.createElement("div");
      logPanel.id = "hck-log-panel";
      const tb = document.createElement("div");
      tb.className = "hck-title-bar";
      const ti = document.createElement("h3");
      ti.textContent = "Logs Detalhados";
      tb.appendChild(ti);
      logPanel.appendChild(tb);
      logContentDiv = document.createElement("div");
      logContentDiv.id = "hck-log-content";
      logPanel.appendChild(logContentDiv);
      const cd = document.createElement("div");
      cd.className = "log-controls";
      const cb = document.createElement("button");
      cb.textContent = "Limpar";
      cb.className = "clear";
      cb.onclick = clearLogs;
      const clb = document.createElement("button");
      clb.textContent = "Fechar";
      clb.onclick = toggleLogPanel;
      cd.appendChild(cb);
      cd.appendChild(clb);
      logPanel.appendChild(cd);
    ...
    function createUI() {
      logToMemory("Iniciando criação da UI V13", "debug");
      if (!document.body) {
        logToMemory("document.body não pronto em createUI", "error");
        return;
      }
      if (document.getElementById("hck-toggle-button")) {
        logToMemory("UI já existe, ignorando.", "debug");
        return;
      }
      try {
        ...
        runButton.onclick = () => {
          if (!isRunning && !isClearing) mainProcessWrapper();
        };
        ...
        clearButton.onclick = () => {
          if (!isRunning && !isClearing) clearFieldsProcessWrapper();
        };
        ...
        logToMemory(`Erro crítico ao criar UI: ${e}`, "error");
        alert("[HCK] Falha ao criar a interface do bookmarklet.");
      }
    }
    ...
    async function insertTextIntoTextarea(
      parentElement,
      textToInsert,
      fieldName
    ) {
      ...
      logToMemory(
        `Tentando ${operation} ${fieldName} ('${shortText}...').`,
        "debug"
      );
      updateStatus(
        `${operation === "limpar" ? "Limpando" : "Inserindo"} ${fieldName}...`
      );

      const textarea = parentElement.querySelector(
        'textarea:not([aria-hidden="true"])'
      );
      if (!textarea) {
        updateStatus(
          `Erro: Textarea para ${fieldName} não encontrado.`,
          "error",
          true
        );
        logToMemory(
          `Textarea para ${fieldName} não encontrado dentro do parentElement.`,
          "error"
        );
        return false;
      }
      ...
      if (success) {
        logToMemory(
          `${fieldName} ${operation === "limpar" ? "limpo" : "inserido"} (M1).`,
          "success"
        );
        updateStatus(
          `${fieldName} ${
            operation === "limpar" ? "limpo" : "inserido"
          } com sucesso.`
        );
        return true;
      }
      ...
      try {
        ...
        const reactPropsKey = Object.keys(textarea).find(
          (key) =>
        ...
        if (reactPropsKey) {
          ...
          if (props && typeof props.onChange === "function") {
            ...
            logToMemory(
              `M2 [${fieldName}]: onChange não é função ou props ausente.`,
              "debug"
            );
          }
        } else {
          logToMemory(
            `M2 [${fieldName}]: Chave ReactProps não encontrada.`,
            "debug"
          );
        }
      } catch (e) {
      ...
      if (success) {
        logToMemory(
          `${fieldName} ${operation === "limpar" ? "limpo" : "inserido"} (M2).`,
          "success"
        );
        updateStatus(
          `${fieldName} ${
      ...
      if (success) {
        logToMemory(
          `${fieldName} ${operation === "limpar" ? "limpo" : "inserido"} (M3).`,
          "success"
        );
        updateStatus(
          `${fieldName} ${
      ...
      if (textarea.value === textToInsert) {
        ...
        logToMemory(
          `Falha nos métodos interativos [${fieldName}]. Tentando valor direto como fallback.`,
          "debug"
        );
        textarea.value = textToInsert;
        await delay(100);
    ...
    async function getAiResponse(prompt, operationDesc) {
      logToMemory(`API Req: ${operationDesc}`, "api");
      updateStatus(`IA: ${operationDesc}...`);
      let attempts = 0;
      while (attempts <= MAX_RETRIES) {
        attempts++;
        logToMemory(
          `API Tentativa ${attempts}/${MAX_RETRIES + 1} para ${operationDesc}`,
          "api"
        );
        try {
          ...
          if (!response.ok) {
            ...
            logToMemory(
              `API Erro ${
                response.status
              } (Tentativa ${attempts}): ${errorBody.substring(0, 150)}...`,
              "error"
            );
            if (attempts > MAX_RETRIES)
              throw new Error(
                `Falha na API (${response.status}) para ${MODEL_NAME}`
              );
            updateStatus(
              `Erro API (${response.status}). Tentando ${attempts}/${
                MAX_RETRIES + 1
              }...`,
              "error"
            );
          ...
          logToMemory(
            `API Resp OK (Tentativa ${attempts}): ${JSON.stringify(
              data
            ).substring(0, 100)}...`,
            "debug"
          );
          ...
          if (!textContent) {
            logToMemory(
              `API Resposta inválida (Tentativa ${attempts}): Ausência de texto.`,
              "error"
            );
            if (attempts > MAX_RETRIES)
              throw new Error("Formato de resposta da API inválido.");
            updateStatus(
              `Erro API (formato). Tentando ${attempts}/${MAX_RETRIES + 1}...`,
              "error"
            );
            await delay(1800 * attempts);
            continue;
          ...
          logToMemory(
            `API ${operationDesc} SUCESSO (Tentativa ${attempts}). Len:${textContent.length}`,
            "success"
          );
          ...
          logToMemory(
            `Exceção na API ${operationDesc} (Tentativa ${attempts}): ${e.message}`,
            "error"
          );
          if (attempts > MAX_RETRIES) {
            updateStatus(`Erro fatal na API: ${e.message}`, "error", true);
            throw e;
          }
          updateStatus(
            `Erro API. Tentando ${attempts}/${MAX_RETRIES + 1}...`,
            "error"
          );
          await delay(1800 * attempts);
        }
    ...
    function extractPageContext() {
      logToMemory("Extraindo contexto da página...", "info");
      ...
      for (const key in selectors) {
        try {
          ...
          if (!context[key]) {
            logToMemory(
              `Contexto para '${key}' vazio (Sel: ${selectors[key]})`,
              "debug"
            );
            if (key === "enunciado") essentialDataMissing = true;
          } else {
            logToMemory(
              `Contexto '${key}': ${context[key]
                .substring(0, 70)
                .replace(/\s+/g, " ")}...`,
              "debug"
            );
          ...
          logToMemory(
            `Erro ao extrair contexto para '${key}': ${e.message}`,
            "error"
          );
          if (key === "enunciado") essentialDataMissing = true;
        }
      ...
      if (essentialDataMissing || !context.enunciado) {
        logToMemory("Erro: Enunciado não encontrado.", "error");
        updateStatus("Erro: Enunciado não extraído.", "error", true);
        return null;
      }
      logToMemory("Contexto extraído.", "success");
      updateStatus("Contexto extraído.");
      return context;
    }

    async function clearFieldsProcess() {
      logToMemory("Limpando campos...", "info");
      updateStatus("Limpando campos...");
      let titleCleared = false,
        bodyCleared = false,
        titleFieldExists = false,
        bodyFieldExists = false,
        titleTextareaParent = null;
      try {
        const titleParentCandidates = document.querySelectorAll(
          TITLE_TEXTAREA_PARENT_SELECTOR
        );
        if (titleParentCandidates.length > 0) {
          ...
          if (titleTextareaParent) {
            ...
            titleCleared = await insertTextIntoTextarea(
              titleTextareaParent,
              "",
              "Título"
            );
        ...
        const allTextareaParents = Array.from(
          document.querySelectorAll(BODY_TEXTAREA_PARENT_SELECTOR)
        ).map((el) => el.parentElement);
        let bodyTextareaParentElement = null;

        if (
          allTextareaParents.length === 1 &&
          (!titleTextareaParent ||
            allTextareaParents[0] !== titleTextareaParent)
        ) {
          bodyTextareaParentElement = allTextareaParents[0];
        } else if (allTextareaParents.length > 1) {
          if (
            titleTextareaParent &&
            allTextareaParents.includes(titleTextareaParent)
          ) {
            bodyTextareaParentElement = allTextareaParents.find(
              (p) => p !== titleTextareaParent
            );
            if (!bodyTextareaParentElement)
              bodyTextareaParentElement =
                allTextareaParents[allTextareaParents.length - 1]; // Fallback
          } else {
            bodyTextareaParentElement =
              allTextareaParents[allTextareaParents.length - 1]; // Assume o último
          }
        } else if (
          allTextareaParents.length === 1 &&
          titleTextareaParent &&
        ...
        if (bodyTextareaParentElement) {
          ...
          bodyCleared = await insertTextIntoTextarea(
            bodyTextareaParentElement,
            "",
            "Corpo"
          );
        ...
        if (!titleFieldExists && !bodyFieldExists) {
          updateStatus("Campos não encontrados para limpar.", "error", true);
          return;
        }

        if (
          (titleFieldExists && titleCleared) ||
          (bodyFieldExists && bodyCleared) ||
          (!titleFieldExists && bodyCleared) ||
          (!bodyFieldExists && titleCleared)
        )
          updateStatus("Campos limpos!", "success", true);
        else {
          let eM = "Erro ao limpar ";
          if (titleFieldExists && !titleCleared) eM += "Título ";
          if (bodyFieldExists && !bodyCleared)
            eM +=
              (titleFieldExists && !titleCleared && bodyFieldExists
                ? "e "
                : "") + "Corpo ";
          if (!titleFieldExists && !bodyFieldExists)
            eM = "Nenhum campo encontrado para limpar."; // Segurança
          updateStatus(eM.trim() + ".", "error", true);
        }
      } catch (e) {
        logToMemory(`Erro limpeza: ${e.message}`, "error");
        updateStatus("Erro ao limpar.", "error", true);
      }
    }
    ...
    async function mainProcess() {
      ...
      const identifierElement = document.querySelector(
        PAGE_IDENTIFIER_SELECTOR
      );
      if (
        !identifierElement ||
        !identifierElement.textContent.includes(PAGE_IDENTIFIER_TEXT)
      ) {
        updateStatus(
          `Erro: Página não é ${PAGE_IDENTIFIER_TEXT}.`,
          "error",
          true
        );
        return;
      }
      updateStatus("Página OK.");
      await delay(150);
...
Seu objetivo é criar uma redação que pareça ter sido escrita por um estudante comum mas faça com que pareça que tenha dominio da língua e capacidade de argumentação, mas que não é um especialista no assunto. A redação deve ser original, coerente e seguir a estrutura dissertativo-argumentativa padrão.
...
2.  **Linguagem e Tom:**
    *   Utilize linguagem formal mas faça com que realmente pareça que um estudante tenha criado essa redação mas de forma, clara, objetiva e em terceira pessoa. Mantenha a impessoalidade.
...
---
Coletânea: ${
        pageContext.coletanea ||
        "Não fornecida. Baseie-se no enunciado e no conhecimento geral."
      }
Enunciado Principal: ${pageContext.enunciado}
Gênero Textual Solicitado: ${
        pageContext.generoTextual || "Dissertativo-argumentativo"
      }
Critérios de Avaliação (se disponíveis): ${
        pageContext.criteriosAvaliacao ||
        "Foco nos critérios comuns de uma redação dissertativo-argumentativa bem estruturada e argumentada."
      }
---
...
`;
      const rawApiResponse = await getAiResponse(
        promptMelhorado,
        "Gerando texto (V13 Melhorado)"
      );
      if (!rawApiResponse) return;

      logToMemory("Analisando resposta IA (V13 Melhorado)...", "info");
      updateStatus("IA: Analisando...");
      let extractedTitle = "";
      let extractedText = "";
      ...
      try {
        ...
        let titleMarker = rawContent.match(
          /TITULO\s*:\s*([\s\S]*?)(?=\n\nTEXTO:|\nTEXTO:|$)/i
        );
        ...
        if (titleMarker && titleMarker[1]) {
          ...
          if (textMarker) {
            extractedText = rawContent
              .substring(textMarker.index + textMarker[0].length)
              .trim();
          } else {
            const potentialTextStart =
              rawContent.indexOf(extractedTitle) + extractedTitle.length;
          ...
          extractedText = rawContent
            .substring(textMarker.index + textMarker[0].length)
            .trim();
          let potentialTitleArea = rawContent
            .substring(0, textMarker.index)
            .trim();
          if (
            potentialTitleArea &&
            !potentialTitleArea.toLowerCase().includes("texto:")
          ) {
            const linesBeforeText = potentialTitleArea.split("\n");
            if (linesBeforeText.length > 0) {
              extractedTitle = linesBeforeText.pop().trim();
              if (
                extractedTitle.toUpperCase() === "TITULO:" &&
                linesBeforeText.length > 0
              ) {
                extractedTitle = linesBeforeText.pop().trim();
          ...
          logToMemory(
            "Marcadores TITULO:/TEXTO: não encontrados. Usando fallback de parsing por linhas.",
            "debug"
          );
          ...
          if (
            ...
            lines[currentLineIndex].toUpperCase().startsWith("TITULO:")
          ) {
            extractedTitle = lines[currentLineIndex]
              .substring("TITULO:".length)
              .trim();
            currentLineIndex++;
            if (
              lines.length > currentLineIndex &&
          ...
          if (
            lines.length > currentLineIndex &&
            lines[currentLineIndex].toUpperCase().startsWith("TEXTO:")
          ) {
            currentLineIndex++;
            if (
              lines.length > currentLineIndex &&
          ...
          if (lines.length > currentLineIndex)
            extractedText = lines.slice(currentLineIndex).join("\n").trim();
          ...
          if (
            ...
            if (
              ...
              !firstLine.endsWith(":") &&
              !firstLine.toUpperCase().startsWith("TEXTO")
            ) {
        ...
        if (extractedTitle.toUpperCase().startsWith("TITULO:"))
          extractedTitle = extractedTitle.substring("TITULO:".length).trim();
        if (extractedText.toUpperCase().startsWith("TEXTO:"))
          extractedText = extractedText.substring("TEXTO:".length).trim();
        ...
        if (
          !extractedTitle &&
          extractedText.toUpperCase().startsWith("TITULO:")
        ) {
        ...
        if (!extractedText && !extractedTitle) {
          throw new Error(
            "Falha crítica: Título e Texto não puderam ser extraídos. Resposta bruta: " +
              rawApiResponse.substring(0, 250)
          );
        }
        ...
        if (extractedTitle)
          ...
          logToMemory(`Título não extraído ou não fornecido pela IA.`, "debug");

        if (extractedText)
          logToMemory(
            `Texto extraído (início): ${extractedText
              .substring(0, 100)
              .replace(/\s+/g, " ")}... (Tamanho: ${extractedText.length})`,
            "success"
          );
        ...
        if (!extractedText) {
          updateStatus(
            "Erro crítico: Corpo do texto não pôde ser extraído da resposta da IA.",
            "error",
            true
          );
          return;
        }
        updateStatus("IA: Resposta analisada com sucesso.");
        await delay(150);
      } catch (e) {
        logToMemory(
          `Erro na análise da resposta da IA: ${
            e.message
          }. Resposta bruta (início): ${rawApiResponse.substring(0, 250)}...`,
          "error"
        );
        updateStatus(
          `Erro ao analisar resposta da IA: ${e.message}`,
          "error",
          true
        );
        return;
      ...
      logToMemory("Inserindo Título...", "info");
      updateStatus("Inserindo Título...");
      const titleTextareaParentCandidates = document.querySelectorAll(
        TITLE_TEXTAREA_PARENT_SELECTOR
      );
      ...
      if (extractedTitle && titleTextareaParent) {
        const titleInserted = await insertTextIntoTextarea(
          titleTextareaParent,
          extractedTitle,
          "Título"
        );
        ...
        updateStatus(
          "Aviso: Título gerado, mas campo de título não encontrado.",
          "error",
          false
        ); // Não é 'true' para não parar tudo.
        logToMemory(
          "Campo de Título não encontrado, mas título foi gerado pela IA.",
          "debug"
        );
      ...
      logToMemory("Inserindo Corpo do Texto...", "info");
      ...
      if (allTextareaParents.length === 1) {
        ...
        if (
          !titleTextareaParent ||
          (titleTextareaParent && allTextareaParents[0] !== titleTextareaParent)
        ) {
          bodyTextareaParentElement = allTextareaParents[0];
        } else if (
          titleTextareaParent &&
          ...
          logToMemory(
            "Apenas um textarea encontrado e já usado para título. Nenhum campo de corpo disponível.",
            "debug"
          );
        }
      } else if (allTextareaParents.length > 1) {
      ...
      if (!bodyTextareaParentElement) {
        updateStatus(
          "Erro crítico: Campo para o corpo do texto não encontrado.",
          "error",
          true
        );
        return;
      ...
      const bodyInserted = await insertTextIntoTextarea(
        bodyTextareaParentElement,
        extractedText,
        "Corpo do Texto"
      );
    ...
    async function clearFieldsProcessWrapper() {
      if (isRunning || isClearing) return;
      isClearing = true;
      if (runButton) runButton.disabled = true;
      if (clearButton) clearButton.disabled = true;
      logToMemory("==== Limpeza Iniciada ====", "info");
      updateStatus("Limpando...", "info");
      try {
        await clearFieldsProcess();
      } catch (e) {
        logToMemory(`Erro Wrapper Limpeza: ${e.message}`, "error");
        updateStatus(`Erro ao limpar: ${e.message}`, "error", true);
      } finally {
        isClearing = false;
        if (runButton) runButton.disabled = false;
        if (clearButton) clearButton.disabled = false;
        updateStatus("Pronto.", "info");
        logToMemory("==== Limpeza Finalizada ====", "info");
      }
    }
    async function mainProcessWrapper() {
      ...
      if (runButton) runButton.disabled = true;
      if (clearButton) clearButton.disabled = true;
      logToMemory("==== Gerar Iniciado ====", "info");
      updateStatus("Iniciando...", "info");
      try {
        await mainProcess();
      } catch (e) {
        logToMemory(`Erro Wrapper Gerar: ${e.message}`, "error");
        updateStatus(`Erro: ${e.message}`, "error", true);
      } finally {
        isRunning = false;
        if (runButton) runButton.disabled = false;
        if (clearButton) clearButton.disabled = false;
        updateStatus("Pronto.", "info");
        logToMemory("==== Gerar Finalizado ====", "info");
      }
    }

    function initialize() {
      logToMemory("Init HCK V13", "info");
      addBookmarkletStyles();
      createUI();
      if (document.getElementById("hck-toggle-button")) {
        updateStatus("Pronto.");
        showToast(`${SCRIPT_NAME} V13 carregado!`, "success", 2500);
      } else {
        logToMemory("UI NF. Retrying.", "error");
    ...
    if (typeof logToMemory === "function")
      logToMemory(`Erro crítico: ${globalError.message}`, "error");
